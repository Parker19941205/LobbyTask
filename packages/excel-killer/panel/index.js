        ...
        window.plugin = new window.Vue({
            ...
            methods: {
                ...
                _initPluginCfg() {
                    ...
                    CfgUtil.initCfg(function (data) {
                        if (data) {
                            ...
                            this.tsFileName = data.tsFileName || "GameTsCfg";
                            ...
                            // this.jsonSavePath = data.jsonPath || ""
                ...
                _initCfgSavePath() {
                    ...
                    let tsSavePath = path.join(pluginResPath, "ts");
                    if (!fs.existsSync(tsSavePath)) {
                        fs.mkdirSync(tsSavePath);
                    }
                ...
                onBtnClickSelectJsonRootPath() {
                    ...
                    if (res !== -1) {
                        let dir = res[0];
                        if (dir !== this.jsonSavePath) {
                            this.jsonSavePath = dir;
                            //chokidar.watch(this.jsonSavePath).on('all', this._watchDir.bind(this));
                            // this._onAnalyzeExcelDirPath(dir);
                            this._saveConfig();
                        }
                    }
                ...
                onBtnClickOpenTsSavePath() {
                    if (fs.existsSync(this.tsSavePath)) {
                        Electron.shell.showItemInFolder(this.tsSavePath);
                        Electron.shell.beep();
                    } else {
                        this._addLog("目录不存在：" + this.tsSavePath);
                    }
                },
                _getTsSaveData(excelData, itemSheet) {
                    //第一行属性字段
                    let title = excelData[0];
                    let descs = excelData[1];
                    let type = excelData[2];
                    let name = itemSheet.sheet
                    //有几个属性 就有几列
                    let attr = "";
                    ...
                    // TODO 保证key的顺序一致性
                    //this.jsFileName
                    let saveFileFullPath = path.join(this.tsSavePath, this.tsFileName + ".ts");
                    let saveStr = "export interface " + name + " {\n" + attr + "\n}\n\n";
                    // fs.writeFileSync(saveFileFullPath, saveStr);
                    fs.appendFileSync(saveFileFullPath, saveStr);
                },
                _getJavaScriptSaveData(excelData, itemSheet) {
                    ...
                    for (let i = 3; i < excelData.length; i++) {
                        ...
                        if (lineData.length === 0) {
                            ...
                            //     this._addLog(title + "[Error] 发现第" + i + "行缺少字段,跳过改行数据配置.");
                            //     continue;
                            // } else
                            // if (lineData.length > title.length) {
                            //     this._addLog(title + "[Error] 发现第" + i + "行多余字段,跳过改行数据配置.");
                            //     continue;
                            // }
                        }
                        let saveLineData = {};
                        ...
                        for (let j = 1; j < title.length; j++) {
                            ...
                            if (value === undefined || value.length <= 0) {
                                // value = " ";
                                // this._addLog(title + "[Error] 发现空单元格:" + itemSheet.name + "*" + itemSheet.sheet + " => (" + key + "," + (i + 1) + ")");
                            } else {
                ...
                // 检测ts配置文件是否存在
                checkTsFileExist() {
                    let saveFileFullPath = path.join(this.jsSavePath, this.jsFileName + ".ts");
                    if (fs.existsSync(saveFileFullPath)) {
                        this.isTsFileExist = true;
                    } else {
                        this.isTsFileExist = false;
                    }
                },
                // 生成配置
                onBtnClickGen() {
                    ...
                    if (this.isMergeJson) {
                        if (this.jsonAllCfgFileName == null || this.jsonAllCfgFileName.length <= 0) {
                            this.jsonAllCfgFileName = "GameJsonCfg";
                        }
                    }
                    if (this.jsFileName == null || this.jsFileName.length <= 0) {
                    ...
                    if (this.tsSavePath) {
                        fsExtra.emptyDirSync(this.tsSavePath);
                    }
                    let jsSaveData = {};// 保存的js数据
                    let jsonAllSaveData = {};// 保存的json数据
                    let tsSaveData = [];//保存的ts数据
                    for (let k in this.excelArray) {
                        ...
                        if (itemSheet.isUse) {
                            ...
                            if (sheetData) {
                                if (sheetData.length > 2) {
                                    ...
                                    // 保存为ts
                                    this._getTsSaveData(sheetData, itemSheet);
                                    tsSaveData.push(itemSheet.sheet)
                                    // 检测重复问题
                    ...
                    let savetsPath1 = path.join(this.tsSavePath, this.tsFileName + ".ts");
                    let saveSt1r = "export class GameData" + " {\n";
                    for (let i = 0; i < tsSaveData.length; i++) {
                        saveSt1r += "   " + tsSaveData[i] + "?: " + tsSaveData[i] + "[];\n"
                    }
                    saveSt1r += "\n}";
                    // fs.writeFileSync(saveFileFullPath, saveStr);
                    fs.appendFileSync(savetsPath1, saveSt1r);
