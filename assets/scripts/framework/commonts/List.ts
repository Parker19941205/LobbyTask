...
export default class List extends cc.Component {
    ...
    //缺省居中
    @property({
        tooltip: CC_DEV && 'Item数量不足以填满Content时，是否顶格显示Item（不支持Grid布局）',
    ...
    @property({
        type: cc.Component.EventHandler,
        tooltip: CC_DEV && '触发滚动到底事件',
    })
    public scrollToBottomEvent: cc.Component.EventHandler = new cc.Component.EventHandler();
    //滚动到顶事件
    @property({
        type: cc.Component.EventHandler,
        tooltip: CC_DEV && '触发滚动到顶事件',
    })
    public scrollToTopEvent: cc.Component.EventHandler = new cc.Component.EventHandler();
    ...
    set numItems(val: number) {
        ...
        // cc.log("this.frameByFrameRenderNum ::", this.frameByFrameRenderNum)
    ...
    _registerEvent() {
        ...
        t.node.on('scroll-to-bottom', t._onScrollBottom, t, true);
        t.node.on('scroll-to-top', t._onScrollTop, t, true);
        //t.node.on('bounce-bottom', t._onBounceBottom, t, true);
        //t.node.on('bounce-top', t._onBounceTop, t, true);
    ...
    _unregisterEvent() {
        ...
        t.node.off('scroll-to-bottom', t._onScrollBottom, t, true);
        t.node.off('scroll-to-top', t._onScrollTop, t, true);
        //t.node.off('bounce-bottom', t._onBounceBottom, t, true);
        //t.node.off('bounce-top', t._onBounceTop, t, true);
    ...
    setTemplateItem(item: any) {
        ...
        let t: any = this;
        t._itemTmp = item;
        t._itemTmp.active = true; // 激活用来实例化的模板item
        ...
        // cc.log("list t._itemSize", t._itemSize)
                ...
                switch (this._verticalDir) {
                    case cc.Layout.VerticalDirection.TOP_TO_BOTTOM: {
                        if (this._customSize) {
                            ...
                            let cs: number = this._customSize[id];
                            // cc.log("有节点修改了大小 计算位置：：", id, fixed, cs)
                    ...
                    case cc.Layout.VerticalDirection.BOTTOM_TO_TOP: {
                        ...
                        if (this.lackCenter) {
                            ...
                            let offset: number = (this.content.height) - (this._allItemSizeNoEdge);
                            bottom += offset;
                        }
                        top = bottom + height;
                        return {
    ...
    _onScrollBottom(){
        cc.log("滚动到底=====>")
        if (this.scrollToBottomEvent) {
            cc.Component.EventHandler.emitEvents([this.scrollToBottomEvent]);
        }
    }
    ...
    _onScrollTop(){
        cc.log("滚动到顶=====>")
         if (this.scrollToTopEvent) {
            cc.Component.EventHandler.emitEvents([this.scrollToTopEvent]);
        }
    }
    ...
    /**
     * 保持修改大小的缓存中 只有一个被选中的数据
     * @param item 
     */
    _markSelectItemSize(item) {
        if (!this._customSize)
            this._customSize = {};
        let val = this._sizeType ? item.height : item.width;
        if (this._customSize[item._listId] != val) {
            if (val == this._itemSize.height) {
                // this._customSize[item._listId] = null
                delete this._customSize[item._listId]
            } else {
                this._customSize = {}//清空上一个备选结点的大小改变
                this._customSize[item._listId] = val;
            }
        }
        // cc.log("强制修改节点大小并且记录 ：：", this._customSize, item._listId, val)
    ...
    update() {
        if (this.frameByFrameRenderNum <= 0 || this._updateDone) {
            // cc.log("this.frameByFrameRenderNum , this._updateDown", this._updateDone)
            return;
        }
    ...
    _createOrUpdateItem(data: any) {
        //cc.log("_createOrUpdateItem !!!!!!", data)
        ...
        if (!item) { //如果不存在
            ...
            // cc.log("item height 1 ::", item.height, item._listId)
            this._resetItemSize(item);
            // cc.log("item height 2 ::", item.height, item._listId)
    ...
    _resetItemSize(item: any) {
        ...
        if (this._customSize && this._customSize[item._listId]) {
            ...
            //cc.log("使用修改后的大小...", size, item._listId)
        } else {
    ...
    getscrollToListIdNow(){
        return this._scrollToListId;
    }
}